...
import { PermissionsContext } from '##shared/context/permission.context';
...
export const useLocation = () => {
  ...
  const [currentLocation, setCurrentLocation] = useState<LocationPosition | null>({ latitude: 0, longitude: 0 });
  const [hasLocation, setHasLocation] = useState<boolean>(false);

  const permissionContext = useContext(PermissionsContext);

  const getCurrentLocation = useCallback((): Promise<LocationPosition | null> => {
