...
type TButtonTypes = | "PRIMARY" | "SECONDARY" | "PRIMARY_LIGHT" | "PRIMARY_OUTLINE" | "TRANSPARENT" | "WHITE_BLUR" | 'GRAY_OUTLINE';
...
export const NetzerPrimaryButton: React.FC<NetzerPrimaryButtonProps> = ({
  ...
  const getButtonSize = useCallback(() => {
    return ({
      [EButtonSizes.NORMAL]: MEDIUM_PADDING,
      [EButtonSizes.SMALL]: SMALL_SPACING
    }?.[size]);
  }, [size])
  ...
  const containerProps = {
    ...
    style: [{ ...styles.button, padding: getButtonSize(), width }, ...getButtonType().background, disabled || loading ? { opacity: 0.7 } : {}],
    activeOpacity: 0.5,
    testID: testId,
    ...props
  };
  ...
  const renderContent = () => {
    ...
    return (
      <View style={styles.container}>
        {glyph && <NetzerIcon glyph={glyph} color={'#E4F2E4'} />}
